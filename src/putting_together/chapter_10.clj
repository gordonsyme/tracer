(ns putting-together.chapter-10
...
(defn go
  ...
  (let [blue1 (colour/colour 0.03 0.23 0.81)
        ...
        checkers (pattern/with-transform
                   (pattern/checks
                     (colour/colour 0.1 1 0.5)
                     (colour/colour 0.27 0.9 0.55))
                   (-> (transform/scaling 0.2 0.2 0.2)
                       (transform/rotate-y (/ Math/PI 6))))
        ...
        floor-pattern (pattern/checked-pattern (pattern/with-transform
                                               ...
                                               (pattern/with-transform
                                                 ...
                                                 (-> (transform/identity)
                                                     (transform/scale 0.2 0.2 0.2)
                                                     (transform/rotate-y (/ Math/PI -3)))))
        left-wall-pattern (let [t (transform/scaling 0.4 0.4 0.4)]
        ...
        floor (shape/with-material
                (plane/plane)
                (material/with-pattern (material/material)
                                       floor-pattern))
        ...
        middle-sphere (-> (sphere/sphere)
                          ...
                          (shape/with-material
                            (-> (material/material)
                                ...
                                (material/with-pattern checkers))))
